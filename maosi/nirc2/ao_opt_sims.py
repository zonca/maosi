...
class GCstars(Scene):
    def __init__(self, label_file="/g/lu/data/gc/source_list/label.dat"):
        ...
        super(self.__class__, self).__init__(x_now, y_now, f_now, mag_now, name_now)
...
class Grid(Scene):
    def __init__(self, n_grid, mag, noise=False):
        ...
        row = np.delete(
            np.arange(-(img_size / 2), (img_size / 2), (img_size / (n_grid + 1))), 0
        )
        grid = np.asarray([row] * n_grid)

        if noise:
            x_noise = np.random.uniform(
                low=-noise / 2, high=noise / 2, size=len(grid.flatten())
            )
        ...
        f_now = Table.Column(
            data=[10 ** ((mag - ZP_mag) / -2.5) * ZP_flux] * (n_grid ** 2), name="Kmag"
        )
...
class PSF_grid_NIRC2_Kp(PSF_grid):
    ...
    def __init__(self, psf, grid_points):
        ...
        grid = grid_points.reshape(
            (wave_shape, int(grid_shape[0]), int(grid_shape[1]), grid_points.shape[1])
        )
...
def test_nirc2_img(psf_grid_raw, psf_grid_pos, outname="tmp.fits"):
    ...
    obs = Observation(
        nirc2, stars, psfgrid, wave_index, background, origin=np.array([512, 512])
    )
