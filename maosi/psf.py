...
class PSF_grid(object):
    ...
    def __init__(
        ...
        psf = psf.reshape(
            (wave_shape, grid_shape[0], grid_shape[1], psf.shape[2], psf.shape[3])
        )
    ...
    def get_local_psf(self, x, y, wave_idx, method="bilinear"):
        ...
        if method == "neighbor":
            ...
            dx = 1.0 * (x - psf_x[xidx_lo]) / (psf_x[xidx_hi] - psf_x[xidx_lo])
            dy = 1.0 * (y - psf_y[yidx_lo]) / (psf_y[yidx_hi] - psf_y[yidx_lo])

            psf_loc = (
                (1 - dx) * (1 - dy) * psf_xlo_ylo
                + (1 - dx) * (dy) * psf_xlo_yhi
                + (dx) * (1 - dy) * psf_xhi_ylo
                + (dx) * (dy) * psf_xhi_yhi
            )
    ...
    def plot_psf_grid(self, wave_idx, psf_size=[50, 50]):
        ...
        img = np.zeros(
            (psf_shape[0] * grid_shape[0], psf_shape[1] * grid_shape[1]), dtype=float
        )
